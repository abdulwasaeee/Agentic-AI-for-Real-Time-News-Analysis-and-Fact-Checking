"""Fact-Check Agent entry point.

Public API for running the LangGraph fact-check pipeline.
...
from models.schemas import EntityRef, FactCheckInput, FactCheckOutput
...
def _claim_to_input(output: PreprocessingOutput, claim_index: int,
                    image_caption: Optional[str], image_url: Optional[str] = None) -> FactCheckInput:
    claim = output.claims[claim_index]
    return FactCheckInput(
        claim_id      = claim.claim_id,
        claim_text    = claim.claim_text,
        entities      = [EntityRef(entity_id=e.entity_id, name=e.name,
...
def fact_check_claim(claim_text: str, source_url: str = "https://unknown.source") -> FactCheckOutput:
    ...
    if fc_output is None:
        ...
        from models.schemas import FactCheckOutput
