...
from orchestration.nodes import (
    cross_modal_check, decompose_claim, emit_output, freshness_check,
    live_search, multi_agent_debate, query_memory, rag_retrieval,
    receive_claim, retrieval_gate, return_cached, synthesize_verdict, write_memory,
)
from orchestration.router import (
    debate_check, freshness_router, retrieval_gate_router, router,
)
...
def build_graph(memory: "MemoryAgent"):
    ...
    g.add_edge("receive_claim",   "decompose_claim")
    g.add_edge("decompose_claim", "query_memory")
    g.add_conditional_edges("query_memory",    router,               {"cache": "freshness_check", "live_search": "retrieval_gate"})
    g.add_conditional_edges("freshness_check", freshness_router,     {"fresh": "return_cached", "stale": "live_search"})
    g.add_conditional_edges("retrieval_gate",  retrieval_gate_router, {"needed": "live_search", "skip": "rag_retrieval"})
    ...
    g.add_conditional_edges("synthesize_verdict", debate_check,      {"skip": "cross_modal_check", "debate": "multi_agent_debate"})
