...
def _openai_vision_check(claim_text: str, image_url: str, api_key: str, model: str) -> dict:
    """Use OpenAI vision (GPT-4o / GPT-4o-mini) to compare image to claim text."""
    try:
        client = OpenAI(api_key=api_key)
        prompt = (
            f"Look at this image carefully. The following news claim has been fact-checked:\n\n"
            f'"{claim_text}"\n\n'
            "1. Briefly describe what the image actually shows (1-2 sentences).\n"
            "2. Does the image conflict with the claim? Answer with conflict=true or conflict=false.\n"
            'Respond as JSON: {"conflict": bool, "explanation": "..."}'
        )
        response = client.chat.completions.create(
            model=model if "vision" in model or model.startswith("gpt-4o") else "gpt-4o-mini",
            messages=[{"role": "user", "content": [
