...
Original schema from Task 1 (Data & Memory Engineer), extended with:
- get_entity_by_name()          — for Entity Tracker lookup by name
- get_claim_count_for_entity()  — for Prediction Agent volume signal
- get_predictions_for_entity()  — for frontend display
- get_entity_ids_for_claims()   — for Task 2 GraphRAG
- get_graph_claims_for_entities() — for Task 2 GraphRAG
"""
...
class GraphStore:
    ...
    def merge_source(self, source_id: str, name: str, domain: str, category: str, base_credibility: float) -> None:
        with self._driver.session() as session:
            session.run(
                """
                ...
                """,
                source_id=source_id, name=name, domain=domain,
                category=category, base_credibility=base_credibility,
            )

    # ── Write: Articles ─────────────────────────────────────────────────

    def create_article(self, article_id: str, title: str, url: str, source_id: str,
                       published_at: datetime, ingested_at: datetime, content_hash: str) -> None:
        with self._driver.session() as session:
            session.run(
                """
                ...
                """,
                article_id=article_id, title=title, url=url, source_id=source_id,
                published_at=published_at.isoformat(), ingested_at=ingested_at.isoformat(),
                content_hash=content_hash,
            )

    # ── Write: Claims with Entities ─────────────────────────────────────

    def create_claims_with_entities(self, claims: list[dict[str, Any]], article_id: str) -> None:
        with self._driver.session() as session:
            for claim in claims:
                session.run(
                    """
    ...
    def create_image_caption(self, caption_id: str, article_id: str, image_url: str, vlm_caption: str) -> None:
        with self._driver.session() as session:
            session.run(
                """
                MATCH (a:Article {article_id: $article_id})
                ...
                """,
                caption_id=caption_id, article_id=article_id,
                image_url=image_url, vlm_caption=vlm_caption,
            )

    # ── Write: Verdicts ─────────────────────────────────────────────────

    def create_verdict(self, verdict_id: str, claim_id: str, label: str, confidence: float,
                       evidence_summary: str, bias_score: float, image_mismatch: bool,
                       verified_at: datetime) -> None:
        print(f"[graph_db.create_verdict] verdict_id={verdict_id!r}  claim_id={claim_id!r}  label={label!r}  conf={confidence:.3f}")
        with self._driver.session() as session:
            session.run(
                """
                MERGE (c:Claim {claim_id: $claim_id})
                ON CREATE SET c.claim_text = '', c.extracted_at = datetime(), c.status = 'pending'
                ...
                """,
                verdict_id=verdict_id, claim_id=claim_id, label=label,
                confidence=confidence, evidence_summary=evidence_summary,
                bias_score=bias_score, image_mismatch=image_mismatch,
                verified_at=verified_at.isoformat(),
            )

    # ── Write: Credibility Snapshots ────────────────────────────────────

    def create_snapshot(self, snapshot_id: str, entity_id: str, credibility_score: float,
                        sentiment_score: float, snapshot_at: datetime) -> None:
        print(f"[graph_db.create_snapshot] entity_id={entity_id!r}  cred={credibility_score:.4f}  sent={sentiment_score:.4f}")
        with self._driver.session() as session:
            session.run(
                """
                MERGE (e:Entity {entity_id: $entity_id})
                ...
                """,
                snapshot_id=snapshot_id, entity_id=entity_id,
                credibility_score=credibility_score, sentiment_score=sentiment_score,
    ...
    def create_prediction(self, prediction_id: str, entity_id: str, prediction_text: str,
                          confidence: float, predicted_at: datetime, deadline: datetime,
        ...
        with self._driver.session() as session:
            session.run(
                ...
                """,
                prediction_id=prediction_id, entity_id=entity_id,
                prediction_text=prediction_text, confidence=confidence,
                predicted_at=predicted_at.isoformat(), deadline=deadline.isoformat(),
                outcome=outcome,
            )
    ...
    def ensure_entity_exists(self, name: str) -> str:
        print(f"[graph_db.ensure_entity_exists] name={name!r}")
    ...
    def backfill_mentions_for_entity(self, entity_id: str, name: str) -> int:
        ...
        Called after ensure_entity_exists so that claims already stored without
        entity extraction get retroactively linked to the entity.
        """
        with self._driver.session() as session:
            result = session.run(
                """
            ...
            print(f"[graph_db.backfill_mentions] entity={name!r}  linked {linked} claim(s)")
            return linked
    ...
    def auto_store_claim_with_entities(self, claim_id: str, claim_text: str,
        ...
        """
        now = datetime.now(timezone.utc).isoformat()
        print(f"[graph_db.auto_store] MERGE Claim  claim_id={claim_id!r}")
        with self._driver.session() as session:
            session.run(
                """
                MERGE (c:Claim {claim_id: $claim_id})
            ...
            print(f"[graph_db.auto_store] Claim node MERGE done")
            if not entity_dicts:
                print(f"[graph_db.auto_store] No entities to link — skipping MENTIONS")
            for entity in entity_dicts:
                print(f"[graph_db.auto_store]   MERGE Entity {entity['name']!r}  id={entity['entity_id']!r}")
                session.run(
                    """
                    MATCH (c:Claim {claim_id: $claim_id})
                    MERGE (e:Entity {entity_id: $entity_id})
                ...
                print(f"[graph_db.auto_store]   Entity + MENTIONS link done")
    ...
    def update_verdict_with_feedback(self, verdict_id: str, correct_label: str,
        ...
        with self._driver.session() as session:
            session.run(
                ...
                """,
                verdict_id=verdict_id, label=correct_label,
                confidence=correct_confidence, note=feedback_note,
            )
